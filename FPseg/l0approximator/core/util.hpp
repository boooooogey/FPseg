...
void backtrace(const RangeList* ranges, const int& N, const double* xprimes, int& k, int* start, int* end, double* value);
...
template <typename T>
void approximate(const int& n, const double* y, const int& N, const double* w, int& k, int* start, int* end, double* value){
    ...
    fs[N-1].max(xprimes[n*N-1], yprime);
    backtrace(ranges, N, xprimes, k, start, end, value);
    T::converter(value, k);
    delete[] fs;
    delete[] gs;
    delete[] xprimes;
